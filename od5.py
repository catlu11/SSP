...
# AIJ DATA
t1 = 2458671.778 # in Julian days
t2 = 2458679.772
t3 = 2458683.774
RA = [RAdecimalToHMS(17.633096*15), RAdecimalToHMS(17.642617*15),
      RAdecimalToHMS(17.661737*15)] # RA coordinates of asteroid at t1, t2, and t3
DEC = [DECdecimalToDMS(-24.906182), DECdecimalToDMS(-22.779707), 
       DECdecimalToDMS(-21.790782)] # DEC coordations of asteroid at t1, t2, and t3
R = [[-2.568180989317604E-01,  9.026400560686667E-01,  3.912553385188193E-01],
      ...
      [-4.465913217555757E-01, 8.375970194890799E-01,  3.630559556728792E-01]] # in equatorial coordinates 
...
def calc_r2(t1, t2, t3, R, RA, DEC):
      ...
      # Adjust for relativistic time difference w/ taus
      tau1 = k*(t1-t2)
      tau3 = k*(t3-t2)
      tau0 = tau3 - tau1
      ...
      while(determine_close(r2, r2o) == False and determine_close(r2dot, r2doto) == False and count < 10000):
...
if __name__ == '__main__':
    r2s = calc_r2(t1, t2, t3, R, RA, DEC)
    calc_elements = calc_orbital_elements(r2s[0], r2s[1])
    errors = calc_percent_error(expected_elements, calc_elements)
    format(expected_elements, calc_elements, errors)
